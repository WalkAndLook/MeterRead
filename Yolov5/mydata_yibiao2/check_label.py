import xml.dom.minidom as xmldom
import os
import shutil

#voc数据集获取所有标签的所有类别数"
annotation_path="Annatotions"
err_xml = "./error_xml"

annotation_names=[os.path.join(annotation_path,i) for i in os.listdir(annotation_path)]

labels = list()
for names in annotation_names:
    xmlfilepath = names
    domobj = xmldom.parse(xmlfilepath)
    # 得到元素对象
    elementobj = domobj.documentElement
    #获得子标签
    subElementObj = elementobj.getElementsByTagName("object")
    for s in subElementObj:
        label=s.getElementsByTagName("name")[0].firstChild.data
        #print(label)
        #if label == "down" or label == "card" or label == "motorcycle":
        #    print("xmlfilepath: ",xmlfilepath)
        #    if os.path.exists(xmlfilepath):
        #        shutil.move(xmlfilepath, err_xml)
        #        shutil.move("./car_img/"+os.path.basename(xmlfilepath)[:-4]+".jpg", err_xml)
        if label not in labels:
            labels.append(label)
            print(labels)
print(labels)
print(len(labels))
