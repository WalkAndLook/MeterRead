...
def autosplit(path=DATASETS_DIR / 'coco128/images', weights=(0.9, 0.1, 0.0), annotated_only=False):
    """ Autosplit a dataset into train/val/test_zbb splits and save path/autosplit_*.txt files
    Usage: from utils.dataloaders import *; autosplit()
    Arguments
        path:            Path to images directory
        weights:         Train, val, test_zbb weights (list, tuple)
...
class HUBDatasetStats():
    ...
    def get_json(self, save=False, verbose=False):
        ...
        for split in 'train', 'val', 'test_zbb':
            if self.data.get(split) is None:
                self.stats[split] = None  # i.e. no test_zbb set
                continue
